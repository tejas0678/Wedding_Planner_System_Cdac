...
export default function FindPlanners() {
  ...
  const fetchPlanners = async () => {
    try {
      ...
      if (cities && cities.length > 0) {
        setCitiesList(['All', ...cities.map((c) => c.name)]);
      } else {
        setCitiesList(['All', 'Mumbai', 'Udaipur', 'Goa', 'Jaipur', 'Delhi', 'Bangalore']);
      }
    } catch (err) {
  ...
  const filteredPlanners = plannersList.filter((planner) => {
    const pName = planner.businessName || planner.name || '';
    const pCity = planner.city || '';
    const matchesSearch = pName.toLowerCase().includes(searchTerm.toLowerCase()) ||
                          pCity.toLowerCase().includes(searchTerm.toLowerCase());
    const matchesCity = selectedCity === 'All' || pCity.toLowerCase() === selectedCity.toLowerCase();
  ...
  return (
    <div className="min-h-screen bg-[#FFF8F9] flex flex-col font-sans">
        ...
        <div className="max-w-4xl mx-auto">
          ...
          <div className="bg-white rounded-full p-2 max-w-2xl mx-auto shadow-lg flex items-center gap-2">
            <div className="flex items-center gap-2 pl-4 flex-1 text-gray-700">
              ...
              <input
                type="text"
                placeholder="Search studio name or city..."
                value={searchTerm}
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 flex-1 w-full">
        ...
        {error && !loading && <ErrorAlert message={error} onRetry={fetchPlanners} />}

        {!loading && !error && filteredPlanners.length === 0 && (
          <EmptyState title="No Planners Found" message="Try tweaking your search term or selected city filter." />
        )}
            ...
            {filteredPlanners.map((planner) => (
              ...
              >
                <div className="relative h-48 overflow-hidden bg-gray-100">
                  <img
                    src={planner.coverImageUrl || planner.avatarUrl || planner.image || "https://images.unsplash.com/photo-1519741497674-611481863552?w=800&auto=format&fit=crop"}
                    alt={planner.businessName || planner.name}
                ...
                <div className="p-6 flex-1 flex flex-col justify-between">
                  <div>
                    <div className="flex items-center gap-1.5 text-xs text-rose-600 font-bold uppercase tracking-wider mb-1">
                      ...
                      <span>{planner.city} • {planner.experience || "8 Years"}</span>
                    </div>
                    <h3 className="font-serif text-xl font-bold text-gray-900 group-hover:text-[#EC3664] transition">
                      {planner.businessName || planner.name}
                    </h3>
                    <p className="text-gray-500 text-xs mt-2 line-clamp-2">
                      {planner.description || planner.tagline || "Premier wedding planning studio."}
                    </p>
                  </div>
                  ...
                  <div className="mt-6 pt-4 border-t border-gray-100 flex items-center justify-between">
                    <div>
                      ...
                      <span className="text-base font-bold text-gray-900">{planner.startingPrice || planner.price || "₹1,50,000"}</span>
                    </div>
